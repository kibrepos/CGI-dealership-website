...
void handleLogin() {
    ...
    if (login_success) {
        ...
        std::ifstream file("C:\\xampp\\htdocs\\login.html");
        if (file.is_open()) {
            std::string line;
            while (std::getline(file, line)) {
                if (line.find("<div class=\"error-msg\" id=\"error-msg\"></div>") != std::string::npos) {
                    std::cout << "<div class=\"error-msg\" id=\"error-msg\">Invalid username or password.</div>";
                } else {
                    std::cout << line << "\n";
                }
            }
